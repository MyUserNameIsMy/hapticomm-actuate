...
LANGUAGES = {
    'en': {
        ...
        "no_device_found": "No device found. Running in demo mode."  # <-- ADDED
    },
    'ru': {
        ...
        "no_device_found": "Устройство не найдено. Демо-режим."  # <-- ADDED
    },
    'kz': {
        ...
        "no_device_found": "Құрылғы табылмады. Демо режимі."  # <-- ADDED
    }
}
...
class HapticTutorApp(tk.Tk):
    def __init__(self):
        ...
        # --- Audio State ---
        self.device_id = None # Changed initial state
    ...
    # --- Haptic Logic ---
    def find_device(self, dev_list, api_list): # <-- MODIFIED
        """Finds a compatible device and returns its ID, or None if not found."""
    ...
    def load_audio_files(self):
        ...
        for key in self.test_letters:
            ...
            if os.path.exists(file_path):
                ...
                raise FileNotFoundError(f"Required signal file not found: {file_path}")
        return signals

    def setup_haptics(self): # <-- MODIFIED
        """Initializes haptic device or sets up demo mode."""
        try:
            self.device_id = self.find_device(sd.query_devices(), sd.query_hostapis())
            self.signals = self.load_audio_files()

            if self.device_id is not None:
                # Device found, set up the output stream
                ...
                # No device found, inform user and proceed in demo mode
                self.status_label.config(text=LANGUAGES[self.language]["no_device_found"], fg="#ff8f00")

            # Enable the start button in both cases (real device or demo mode)
    ...
    def play_signal(self, signal_key): # <-- MODIFIED
        """Plays a haptic signal or simulates it if in demo mode."""
        if self.is_playing: return
        if signal_key not in self.signals: return

        self.is_playing = True
        self.set_ui_state("playing")

        if self.out_stream:
            # Real device is connected, play the signal
            self.current_signal_data = self.signals[signal_key]
            self.X = 0
            self.out_stream.start()
        else:
            # No device, simulate the signal playback with a 1.5-second delay
            SIMULATED_DURATION_MS = 1500
            self.after(SIMULATED_DURATION_MS, self.on_stream_finished)

    def on_stream_finished(self):
        self.is_playing = False
        self.after(10, lambda: self.set_ui_state(self.app_state))
    ...
    def next_trial(self):
        self.set_ui_state("idle") # Ensure buttons are disabled before trial
        ...
        for btn in self.answer_buttons.values(): btn.config(state="disabled")
        self.replay_game_button.config(state="disabled")
    ...
    def check_answer(self, chosen_letter):
        ...
        if chosen_letter == self.correct_answer:
            ...
            self.after(1500, self.next_trial)
        else:
    ...
    def end_game(self):
        ...
        self.learn_button.pack(pady=10)
        self.learn_button.config(state="normal")
        self.learn_button.config(command=self.start_learning) # Reset command

    def set_ui_state(self, state):
        is_active = not self.is_playing
        
        if state == "learning":
            self.next_learn_button.config(state="normal" if is_active else "disabled")
            self.replay_learn_button.config(state="normal" if is_active else "disabled")
        elif state == "testing":
            for btn in self.answer_buttons.values():
                btn.config(state="normal" if is_active else "disabled")
            self.replay_game_button.config(state="normal" if is_active else "disabled")
